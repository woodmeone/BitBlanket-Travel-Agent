"""Unit tests for the explicit contract seam around the runtime execution flow."""
...
from agent.travel_agent.graph import runtime_flow
from agent.travel_agent.runtime.runtime_driver import DefaultRuntimeDriver


def test_stream_supervisor_run_uses_explicit_contract(monkeypatch):
    """Runtime flow stream entrypoint should consume supervisor request/context directly."""
    ...
    async def _collect():
        return [event async for event in runtime_flow.stream_supervisor_run(request=request, context=context)]
...
def test_generate_supervisor_plan_preview_uses_explicit_contract(monkeypatch):
    """Runtime flow preview entrypoint should consume supervisor request/context directly."""
    observed: dict[str, object] = {}
    ...
    preview = runtime_flow.generate_supervisor_plan_preview(request=request, context=context)
...
def test_stream_graph_source_delegates_event_assembly_to_runtime_event_emitter(monkeypatch):
    """Runtime flow helper should delegate normalized event assembly to the emitter layer."""
    ...
    monkeypatch.setattr(runtime_flow, "SupervisorEventEmitter", _FakeEmitter)
    ...
    async def _collect():
        return [
            event
            async for event in runtime_flow._stream_graph_source(
                source=source,
                user_message="plan a trip",
                session_id="session-1",
                persist_memory=True,
...
def test_default_runtime_driver_delegates_to_runtime_flow(monkeypatch):
    """Runtime driver should delegate to runtime-flow entrypoints without re-packing kwargs."""
    observed: dict[str, object] = {}
    ...
    monkeypatch.setattr(runtime_flow, "stream_supervisor_run", _fake_stream_supervisor_run)
    monkeypatch.setattr(
        runtime_flow,
...
def test_collect_supervisor_tool_health_diagnostics_uses_explicit_contract(monkeypatch):
    """Runtime flow diagnostics should normalize raw monitoring dictionaries into one contract."""
    ...
    diagnostics = runtime_flow.collect_supervisor_tool_health_diagnostics()
...
def test_default_runtime_driver_returns_tool_health_contract(monkeypatch):
    """Runtime driver should delegate tool-health diagnostics to the typed runtime-flow seam."""
    ...
    diagnostics = DefaultRuntimeDriver().get_tool_health_diagnostics()
