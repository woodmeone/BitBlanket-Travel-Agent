...
> Status: 历史路线图。本文保留为架构演进记录，不作为当前实现真相源。
> 当前维护请优先看 [../README.md](../README.md)、[system-architecture.md](system-architecture.md)、[data-storage.md](data-storage.md)、[infrastructure-foundations.md](infrastructure-foundations.md)。

## 当前维护者速查

这份历史路线图真正需要记住的只剩 4 条：

| 主题 | 当前已落地基线 |
| --- | --- |
| Runtime seam | `AgentRuntime -> runtime_driver -> runtime_flow` 已成为稳定主链，runtime request / preview / diagnostics 都走显式 contract。 |
| Supervisor / Subagents / Skills | `supervisor/`、`subagents/`、`skills/registry.py` 已成为真实目录边界，能力选择不再只埋在 prompt 里。 |
| Artifact-first 消费 | `execution receipt`、artifact patch 和 share / replay / delivery bundle 已能进入 SSE、persisted diagnostics 和前端恢复链。 |
| 治理与评测 | `skills_market_audit`、subagent scorecard、release harness scorecard 已把 skill 治理、评测和发版证据收口到同一套门禁面。 |

当前维护时，这份文档只适合回答“为什么项目会沿着 `Supervisor -> Subagents -> Skills -> Artifact-first` 这条路演进”；具体实现真相源请回到 [system-architecture.md](system-architecture.md)、[infrastructure-foundations.md](infrastructure-foundations.md)、[data-storage.md](data-storage.md) 和 [../reference/project-structure.md](../reference/project-structure.md)。

下面的分阶段内容保留为历史架构推演与评审素材，不建议再按 checklist 式阅读。
...
1. Agent 应用层
   - 也就是 Backend API、ChatService、SSE、Artifact、工作流、会话系统如何围绕 Agent 升级。
2. Agent 架构层
...
- [state.py](../../agent/travel_agent/graph/state.py)
- [nodes.py](../../agent/travel_agent/graph/nodes.py)
- [travel_tools.py](../../agent/travel_agent/tools/travel_tools.py)
- [memory_integration.py](../../agent/travel_agent/graph/memory_integration.py)
...
[nodes.py](../../agent/travel_agent/graph/nodes.py) 当前同时承担了：
...
[travel_tools.py](../../agent/travel_agent/tools/travel_tools.py) 当前更像 `tool adapters + mock fallback + provider bridge`，但还不是 skill 层。
...
[builder.py](../../agent/travel_agent/graph/builder.py) 里的图虽然已经有分支和回环，但本质还是单一主 Agent 图。
...
[state.py](../../agent/travel_agent/graph/state.py) 里的 `AgentState` 已经包含很多字段，这本身没有问题，但继续无节制增长会带来：
...
当前 [nodes.py](../../agent/travel_agent/graph/nodes.py) 里的 `verify_node` 和 `self_check_node` 已经体现出质量闭环思想，但它们还没有真正成为“独立 evaluator”。
...
[memory_integration.py](../../agent/travel_agent/graph/memory_integration.py) 已经很好，但当前 memory 主要面向整个 session。
...
这一层当前已经有雏形，主要就在 [travel_tools.py](../../agent/travel_agent/tools/travel_tools.py) 和相关 provider 适配里。
...
| 当前文件 | 问题 | 未来建议落点 |
| --- | --- | --- |
| [graph/builder.py](../../agent/travel_agent/graph/builder.py) | 当前承担全部主图装配 | 收口为更薄的组图入口，并新增 `supervisor/graph_builder.py` |
| [graph/nodes.py](../../agent/travel_agent/graph/nodes.py) | 过大、职责过多 | 拆到 `supervisor/` + `subagents/` |
| [graph/state.py](../../agent/travel_agent/graph/state.py) | 单大状态对象 | 新增 `supervisor_state.py`、artifact state |
| [tools/travel_tools.py](../../agent/travel_agent/tools/travel_tools.py) | tools 与 skill 未分层 | 保留 tool adapter，新增 `skills/registry.py` |
| [graph/memory_integration.py](../../agent/travel_agent/graph/memory_integration.py) | memory 主要面向整轮 session | 迁移到 `memory/`，拆 conversation / working / artifact memory |
| [graph/persistent_checkpointer.py](../../agent/travel_agent/graph/persistent_checkpointer.py) | 能力可保留 | 继续复用，提供 runtime-level 接口 |
| [chat_service.py](../../backend/moyuan_web/services/chat_service.py) | 直接耦合 graph 细节 | 改为调 `agent_runtime.py` |
...
- 对前端现有 SSE 协议保持兼容
- [chat_service.py](../../backend/moyuan_web/services/chat_service.py) 不再直接依赖底层 graph 细节
...
- `backend/moyuan_web/routes/artifact.py`
- `backend/moyuan_web/services/artifact_service.py`
...
- [system-architecture.md](system-architecture.md)
- [api-reference.md](../reference/api-reference.md)
- [04-agent-core-tools-memory-checkpoint.md](../teaching/04-agent-core-tools-memory-checkpoint.md)
- [testing-guide.md](../testing/testing-guide.md)

### Phase 3

- [system-architecture.md](system-architecture.md)
- [product-requirements.md](../product/product-requirements.md)
- [api-reference.md](../reference/api-reference.md)
- [02-chat-mainline-and-frontend.md](../teaching/02-chat-mainline-and-frontend.md)
...
2. 把 [nodes.py](../../agent/travel_agent/graph/nodes.py) 拆成 supervisor / subagent 可迁移结构
...
- [`frontend/src/utils/agentArtifacts.ts`](../../frontend/src/utils/agentArtifacts.ts)
...
- [`agent/travel_agent/subagents/budget.py`](../../agent/travel_agent/subagents/budget.py)
  - promotes budget estimation into a real domain subagent with dedicated artifact patches
- [`backend/moyuan_web/services/artifact_service.py`](../../backend/moyuan_web/services/artifact_service.py)
  - exposes persisted trip artifacts as an application-layer service boundary
- [`backend/moyuan_web/routes/artifact.py`](../../backend/moyuan_web/routes/artifact.py)
  - adds `GET /api/artifacts/{session_id}/latest` and `GET /api/artifacts/{session_id}/history` as public artifact retrieval endpoints
- [`frontend/src/services/api/artifactClient.ts`](../../frontend/src/services/api/artifactClient.ts)
