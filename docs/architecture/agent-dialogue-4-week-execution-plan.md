...
> Status: 已完成的阶段执行稿。本文保留为历史执行记录，不作为当前实现真相源。
...
这份历史执行稿真正需要保留的只有“4 周最后交付了什么”：

| 周次 | 历史目标 | 当前沉淀结果 |
| --- | --- | --- |
| Week 1 | 稳定性与可观测性打底 | `plan_preview` 校验、工具健康诊断、回放脚本、基础稳定性回归已成为当前基线。 |
| Week 2 | 验证闭环与时效治理 | stale 刷新、证据化回答、安全策略配置化、SSE metadata 扩展已沉淀到当前实现。 |
| Week 3 | 评测体系与趋势化运营 | benchmark / golden / trend / intent 聚合诊断成为常规质量面。 |
| Week 4 | CI 门禁与发布收口 | CI 质量门禁、配置文档、发布前全量回归和阶段总结已经转成当前治理基线。 |

当前维护时，这份文档只适合回答“这轮对话链路强化最后落成了哪些治理结果”；具体实现真相源请看 [system-architecture.md](system-architecture.md)、[infrastructure-foundations.md](infrastructure-foundations.md)、[backend-database-devops-maintenance-plan.md](backend-database-devops-maintenance-plan.md) 和 [../testing/testing-guide.md](../testing/testing-guide.md)。

下面的周排期表保留为历史执行记录，适合复盘，不适合作为当前维护入口。
...
| W1-3 | 增强工具健康诊断（SLO 结构化输出） | `backend/moyuan_web/services/chat_service.py`（聚合 run 指标）、`backend/moyuan_web/routes/health.py`（响应模型扩展） | `GET /api/health/tools` 增加 `slo`、`intent_aggregate`、`window_minutes` | 健康接口可直接看超时率/失败率/fallback 率是否越线 |
| W1-4 | 失败回放工具 v1（基于 checkpoint） | 新增 `scripts/agent_replay.py`；必要时补 `agent/travel_agent/graph/persistent_checkpointer.py` 读取能力 | CLI：`python scripts/dev.py agent-replay --replay-session-id ... --replay-db ...` | 线上失败会话可本地复现并输出 replay 报告 |
| W1-5 | 补齐稳定性回归测试 | `tests/test_agent_p0_guardrails_unit.py`、`tests/test_chat_stream_local.py`、新增 `tests/test_agent_replay_script_unit.py` | `python scripts/dev.py backend-test --pytest-slice all` | 新增用例覆盖：计划预检、健康聚合、回放脚本 |
...
| W2-4 | SSE 元数据补全验证状态 | `backend/moyuan_web/services/chat_service.py`（metadata 拼装）、`frontend/src/services/api/chatClient.ts` + `frontend/src/services/api/chatStreamParser.ts`（解析元数据）、`frontend/src/types/index.ts`（类型扩展） | SSE `metadata` 增加 `verification_passed`、`stale_result_count`、`fallback_steps` | 前端可展示“是否通过验证”和“是否有过期数据” |
...
| W3-3 | 意图维度运行统计接口 | `backend/moyuan_web/services/chat_service.py`（按 intent 聚合窗口）、`backend/moyuan_web/routes/health.py`（新增子路由） | 新增 `GET /api/health/tools/intents` | 可直接查询每种 intent 的成功率/超时率/fallback 率 |
...
| W3-5 | 测试与报告流水线 | `tests/test_agent_benchmark_script_unit.py`、新增 `tests/test_agent_benchmark_trend_script_unit.py` | `python scripts/dev.py benchmark-report` + `python scripts/dev.py benchmark-trend` 可稳定产报告 | 报告生成纳入可重复流程 |
...
| W4-4 | 发布前全量回归 | `tests/`、`frontend/tests/`、`scripts/` | 命令：`python scripts/dev.py backend-test --pytest-slice all`、`python scripts/dev.py frontend-test`、`python scripts/dev.py golden-report`、`python scripts/dev.py quality-gate` | 通过发布前检查清单，形成周报 |
...
- BE-API：`backend/moyuan_web/services/*`、`backend/moyuan_web/routes/*`
- FE：`frontend/src/components/*`、`frontend/src/services/api/chatClient.ts`、`frontend/src/services/api/chatStreamParser.ts`、`frontend/src/types/index.ts`
