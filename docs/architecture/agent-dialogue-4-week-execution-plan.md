...
|---|---|---|---|---|
| W1-1 | 计划阶段工具注册预检（防 `TOOL_NOT_FOUND`） | `agent/src/graph/nodes.py`（`plan_node`、执行前校验函数）、`agent/src/graph/state.py`（新增校验结果字段） | `POST /api/chat/stream` 的 `plan_preview` 增加 `validation_status`/`validation_errors`（仅 plan 模式） | 计划阶段即拦截未注册工具；执行阶段不再出现 `TOOL_NOT_FOUND` |
...
| W1-3 | 增强工具健康诊断（SLO 结构化输出） | `web/src/services/chat_service.py`（聚合 run 指标）、`web/src/routes/health.py`（响应模型扩展） | `GET /api/health/tools` 增加 `slo`、`intent_aggregate`、`window_minutes` | 健康接口可直接看超时率/失败率/fallback 率是否越线 |
| W1-4 | 失败回放工具 v1（基于 checkpoint） | 新增 `scripts/agent_replay.py`；必要时补 `agent/src/graph/persistent_checkpointer.py` 读取能力 | CLI：`python scripts/agent_replay.py --session-id ... --db ...` | 线上失败会话可本地复现并输出 replay 报告 |
...
| ID | 任务 | 文件级改造 | 接口/脚本级改造 | 验收标准 |
|---|---|---|---|---|
| W2-1 | 关键工具 stale 自动刷新（weather/hotels） | `agent/src/graph/nodes.py`（`verify_node/execute_node` 刷新策略）、`agent/src/tools/travel_api.py`（支持 bypass cache）、`agent/src/tools/travel_tools.py`（透传 refresh 参数） | tool `_meta` 增加 `refresh_attempted`、`refresh_success` | stale 数据优先刷新；无法刷新时输出明确降级说明 |
| W2-2 | 答案证据化模板强化 | `agent/src/graph/prompt_templates.py`、`agent/src/graph/nodes.py`（`answer_node` 证据段渲染） | `POST /api/chat/stream` 最终回答要求显式包含 `source/fetched_at`（高风险场景强制） | 抽样问答中高风险场景证据覆盖率 >= 98% |
| W2-3 | 安全策略分级与配置化 | `agent/src/graph/nodes.py`（注入规则分级）、`agent/src/graph/runtime_config.py`（新增 guardrail 阈值） | 新增环境变量写入 `docs/reference/configuration-reference.md` | 注入样例拦截率上升，且误杀率可控（通过 golden 子集验证） |
| W2-4 | SSE 元数据补全验证状态 | `web/src/services/chat_service.py`（metadata 拼装）、`frontend/src/services/api.ts`（解析元数据）、`frontend/src/types/index.ts`（类型扩展） | SSE `metadata` 增加 `verification_passed`、`stale_result_count`、`fallback_steps` | 前端可展示“是否通过验证”和“是否有过期数据” |
...
| W3-3 | 意图维度运行统计接口 | `web/src/services/chat_service.py`（按 intent 聚合窗口）、`web/src/routes/health.py`（新增子路由） | 新增 `GET /api/health/tools/intents` | 可直接查询每种 intent 的成功率/超时率/fallback 率 |
...
| W4-2 | 运行时配置收口与默认值审计 | `agent/src/graph/runtime_config.py`、`docs/reference/configuration-reference.md` | 配置项分组：可靠性/时效性/安全/成本 | 配置可灰度启停，避免硬编码 |
...
- BE-Agent：`agent/src/graph/*`、`agent/src/tools/*`、`scripts/*`
- BE-API：`web/src/services/*`、`web/src/routes/*`
