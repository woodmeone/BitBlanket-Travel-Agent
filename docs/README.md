...
- `scripts/runtime_contract_audit.py`: 审计 `AgentRuntime -> legacy_bridge -> legacy_runtime -> runtime_sources/runtime_event_emitters` 的 typed seam 是否保持显式 contract，并阻止 memory source-state 或 event assembly 漂回 shim
