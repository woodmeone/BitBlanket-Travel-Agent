...
- `agent/travel_agent/runtime_event_emitters.py` 现在承接 legacy supervisor stream 的 `stage / reasoning / chunk / tool_start / tool_end / done` 合同化事件装配，让 `legacy_runtime.py` 不再内联维护一整套阶段进度、done payload 与截断补全文案逻辑
