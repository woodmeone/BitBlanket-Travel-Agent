...
- `agent/travel_agent/contracts/supervisor_events.py` 现在把 legacy graph 归一化后的 `stage / reasoning / chunk / tool_start / tool_end / done` 事件收口成显式 contract，`agent/travel_agent/graph/builder.py` 不再直接手搓大段 runtime event 字典
- `agent/travel_agent/graph/legacy_runtime.py` 现在统一承接旧 graph 的 `run / stream / stream_with_memory / preview / diagnostics` 兼容执行入口，`agent/travel_agent/graph/builder.py` 已进一步退化为更纯的组图与编译模块
- `agent/travel_agent/graph/legacy_runtime.py` 现在也直接消费 `SupervisorRunRequest / SupervisorPlanPreviewRequest / SupervisorRuntimeContext`，由 `stream_supervisor_run()` 和 `generate_supervisor_plan_preview()` 统一接收 contract；`agent/travel_agent/runtime/legacy_bridge.py` 不再手工回填 streaming / preview 所需的一长串 kwargs
- `agent/travel_agent/runtime_sources.py` 现在承接 memory-aware graph / preview source 的状态装配、默认 checkpointer 选择和 supervisor request/context 到 legacy source 的映射，让 `legacy_runtime.py` 收口成更窄的 shim 与事件归一化层
- `agent/travel_agent/contracts/supervisor_orchestration.py` 现在也显式定义 `SupervisorPlanPreview` 结果 contract；`agent/travel_agent/graph/legacy_runtime.py` 与 `agent/travel_agent/runtime/legacy_bridge.py` 会先在 seam 内返回 contract，`agent/travel_agent/runtime/agent_runtime.py` 再统一转成 enriched preview dict，对外兼容 preview/artifact 消费不变
- `agent/travel_agent/contracts/supervisor_orchestration.py` 现在还显式定义 `SupervisorToolHealthDiagnostics / SupervisorToolHealthEntry`；`agent/travel_agent/graph/legacy_runtime.py` 与 `agent/travel_agent/runtime/legacy_bridge.py` 会先返回 typed diagnostics contract，`agent/travel_agent/runtime/agent_runtime.py` 再统一合并 skills/subagents 元数据，对外 health/diagnostics 接口保持兼容
...
- `agent/travel_agent/runtime/legacy_bridge.py` 现在把 `AgentRuntime -> legacy graph.builder` 的 streaming / preview / diagnostics 兼容调用集中成显式 bridge，`AgentRuntime` 主编排不再直接 import 旧入口函数，后续继续替换 legacy graph 时可以只收口 bridge 层
...
- `scripts/runtime_contract_audit.py` 现在会守住 `AgentRuntime -> legacy_bridge -> legacy_runtime -> runtime_sources` 这条 typed seam，避免 runtime 兼容层重新退化回散落的 loose kwargs、直接 graph import 或直接在 shim 内组装 memory state
