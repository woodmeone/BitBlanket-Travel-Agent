"""File-backed repository for full agent-memory session snapshots."""
...
class FileMemorySessionRepository(MemorySessionRepository):
    """Persist memory snapshots to one JSON file plus a hot-backup copy."""

    def __init__(self, persist_path: str | None, *, backup_suffix: str = ".bak") -> None:
        """Store the primary snapshot path and backup suffix."""
    ...
    def enabled(self) -> bool:
        """Return whether file-backed persistence is configured."""
    ...
    @staticmethod
    def _load_snapshot_from_file(path: str) -> dict[str, Any] | None:
    ...
    def _atomic_write_json(self, path: str, payload: dict[str, Any]) -> None:
        """Persist JSON payload atomically via temp file, fsync, and replace."""
